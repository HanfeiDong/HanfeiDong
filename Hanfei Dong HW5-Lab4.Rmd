...
Research Question:
Is there a relationship between mental health days and childhood maltreatment?
Is there a statistically significant difference in the number of mental health days between those who reported "once" and those who reported "not sure"?
...
``` {r}
sub_data_once <- subset(brfss_ACE, ACETOUCH == "once" & !is.na(MENTHLTH))
sub_data_dont_know <- subset(brfss_ACE, ACETOUCH == "dont know not sure" & !is.na(MENTHLTH))
```
...
There was a significant effect of the independent variable ACETOUCH (categories of childhood abuse experiences) on the dependent variable MENTHLTH (mental health days), F(4, 47077) = 431.8, p < 2e-16.
Reject the null hypothesis. This indicates that there is a significant difference in the number of mental health days between categories of childhood abuse experiences.

``` {r}
correlation <- cor(brfss_ACE$MENTHLTH, brfss_ACE$ACETOUCH_recode, use = "complete.obs")
correlation
```

The Pearson correlation coefficient between the number of mental health days (MENTHLTH) and the recoded variable for childhood abuse experience (ACETOUCH_recode) was 0.188.
This implies that there is some correlation between the degree of childhood abuse experience (recoded as a variable between 0 and 1) and the number of mental health days, but it does not fully explain the variation in the number of mental health days.
